...
class ApprovalMixin:
    ...
    async def _cb_video_action(self, query, action: str, profile_id: str, video_id: str) -> None:
        ...
        # Trigger download/delete for local playback
        is_now_approved = (
            action in ("approve", "approve_edu", "approve_fun")
            or (action in ("allowchan", "allowchan_edu", "allowchan_fun", "autoapprove")
                and video['status'] == 'pending')
        )
        if is_now_approved and self.on_video_approve:
            import asyncio
            asyncio.create_task(self.on_video_approve(video_id, profile_id))
        elif action in ("deny", "revoke", "blockchan") and self.on_video_revoke:
            import asyncio
            asyncio.create_task(self.on_video_revoke(video_id))
