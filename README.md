<p align="center">
  <img src="web/static/brg-logo.png" alt="67guard" width="300">
</p>
...
67guard puts you in control of what your kids watch on YouTube — without standing over their shoulder.
...
```
Kid's Tablet ----> 67guard Server ----> Router ----> Telegram Cloud ----> Parent's Phone
                        |
                  Tablet lockdown (Family Link / Screen Time)
                  only 67guard PWA allowed
```

1. Kid opens 67guard on their tablet and searches for a video
...
<img src="docs/screenshots/screenshots-combined.png" alt="67guard on desktop, tablet, and phone — homepage, search, watch, and activity views" width="900">
...
67guard removes the algorithm entirely. There's no autoplay, no "up next" sidebar, no recommendation engine pulling him deeper. He searches for what he wants, I approve or deny, and the video plays and stops. Done. No rabbit holes.
...
**Unraid:** Search for **67guard** in Community Applications, or download the template manually:
```bash
...
**Important:** You'll also want to [lock down the kid's device](docs/setup.md#step-5-lock-down-the-kids-device) so they can't bypass 67guard by opening YouTube directly.
...
> **Network note:** 67guard runs on your home network. Your child's device needs to be on the same network to access the web UI. You can approve/deny from anywhere via Telegram — you don't need to be home for that part.
