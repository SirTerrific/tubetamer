...
async def watch_video(request: Request, video_id: str):
    ...
    # Determine if local playback is available
    local_playback_enabled = getattr(state, "local_playback_enabled", False)
    local_playback = False
    local_stream_url = ""
    download_status_val = ""
    if local_playback_enabled:
        downloader = getattr(state, "video_downloader", None)
        if downloader and downloader.is_downloaded(video_id):
            local_playback = True
            local_stream_url = f"/api/stream/{video_id}"
        else:
            # File not ready — show download waiting page
            raw_status = cs.get_download_status(video_id)
            # Trigger download if not already queued/downloading
            dl_cb = getattr(state, "download_on_approve", None)
            if dl_cb and raw_status not in ("downloading", "ready"):
                await dl_cb(video_id, cs.profile_id)
            download_status_val = raw_status if raw_status in ("downloading",) else "pending"
            w_cfg = request.app.state.web_config
            poll_interval = w_cfg.poll_interval if w_cfg else 3000
            return templates.TemplateResponse(request, "downloading.html", {
                **base,
                "video": video,
    ...
    return templates.TemplateResponse(request, "watch.html", {
        **base,
        ...
        "subtitle_tracks": (downloader.subtitle_files(video_id)
                            if local_playback and downloader else []),
    })
