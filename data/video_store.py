"""
SQLite-backed video storage for TubeTamer.
...
class VideoStore:
    ...
    def _create_tables(self) -> None:
        ...
        # Add download_status column if missing (local playback feature)
        self._add_column_if_missing("videos", "download_status", "TEXT DEFAULT ''")

    _ALLOWED_TABLES = {"channels", "videos", "watch_log", "settings", "search_log", "word_filters", "profiles"}
    _ALLOWED_COLUMNS = {"channel_id", "handle", "category", "is_short", "profile_id", "avatar_icon", "avatar_color", "yt_view_count", "resume_seconds", "download_status"}
    ...
    # --- Download status (local playback) ---

    def set_download_status(self, video_id: str, status: str, profile_id: str = "default") -> None:
        """Set the download status for a video."""
        with self._lock:
            self.conn.execute(
                "UPDATE videos SET download_status = ? WHERE video_id = ? AND profile_id = ?",
                (status, video_id, profile_id),
            )
            self.conn.commit()

    def get_download_status(self, video_id: str, profile_id: str = "default") -> str:
        """Get the download status for a video."""
        with self._lock:
            cursor = self.conn.execute(
                "SELECT download_status FROM videos WHERE video_id = ? AND profile_id = ?",
                (video_id, profile_id),
            )
            row = cursor.fetchone()
            return row["download_status"] if row and row["download_status"] else ""

    def get_videos_by_download_status(self, status: str) -> list[dict]:
        """Get all videos with a given download status (across all profiles)."""
        with self._lock:
            cursor = self.conn.execute(
                "SELECT video_id, profile_id FROM videos WHERE download_status = ? AND status = 'approved'",
                (status,),
            )
            return [dict(row) for row in cursor.fetchall()]

    def is_video_approved_anywhere(self, video_id: str) -> bool:
        """Check if a video is approved in any profile."""
        with self._lock:
            cursor = self.conn.execute(
                "SELECT 1 FROM videos WHERE video_id = ? AND status = 'approved' LIMIT 1",
                (video_id,),
            )
            return cursor.fetchone() is not None

    def clear_download_status(self, video_id: str) -> None:
        """Reset download_status to empty for all profiles (used after file cleanup)."""
        with self._lock:
            self.conn.execute(
                "UPDATE videos SET download_status = '' WHERE video_id = ?",
                (video_id,),
            )
            self.conn.commit()
